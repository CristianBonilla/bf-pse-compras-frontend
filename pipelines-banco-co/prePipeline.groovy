    ...
    stage('Dependency Check Analysis') {
        catchError(buildResult: 'UNSTABLE', stageResult: 'FAILURE') {
            docker.image('hub.fif.tech/base/plugins/dependency-check-v6:latest').inside('--entrypoint="" -u root'){
                ...
                env.CI_JOB="true"
                env.DC_TARGET_LANG="python" // Cambiar por lenguaje correspondiente.
                sh '''
...
node('cislaveazure') {
    ...
    stage('Deploying Stack') {
      sh '''
        ...
        docker stack deploy --with-registry-auth -c ${WORKSPACE}/deploy/docker-stack.yml  gateway-compras-backend
      '''
    }
 }
}

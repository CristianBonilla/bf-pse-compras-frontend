  ...
  node('azure-slave') {
    ...
    stage('Dependency Check Analysis') {
        catchError(buildResult: 'UNSTABLE', stageResult: 'FAILURE') {
            docker.image('hub.fif.tech/base/plugins/dependency-check:latest').inside('--entrypoint="" -u root'){
    ...
    stage('Sonarqube Analysis') {
      docker.image('hub.fif.tech/base/plugins/sonar-scanner-cli:v4.6.2.2472').inside('--entrypoint="" -u root')
	  {
    ...
    //     withCredentials([usernamePassword(credentialsId: 'banco-co-gateway-pagos-prod-pipeline', passwordVariable: 'SECRET_ID', usernameVariable: 'ROLE_ID')]) {
    //       script {
    ...
    //         sh '''
    //           vault read -field=auth_config kv/clusters/banco-co-gateway-pagos-prod-cluster/shared/automated/harbor-banco-co-gateway-pagos > config.json
    //         '''
    //       }
    //     }
    //   }
...
node('cislaveazure') {
    ...
    stage('Deploying Stack') {
      sh '''
        eval $(docker-machine env banco-co-gateway-pagos-prod-cluster --shell sh)
        set -a
